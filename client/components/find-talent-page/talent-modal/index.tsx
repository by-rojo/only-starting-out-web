...
const TalentModal: React.FC<PropsWithChildren<{}>> = ({ children }) => {
  const {
    push,
    pathname,
    query: { id, ...query },
  ...
  return (
    <>
      <BlowUpModal
        ...
        className={classNames(styles.talentModal)}
      >
        <CardTitle className="mx-3 my-4 fw-bold">Talent Name</CardTitle>
        <CardBody className="overflow-auto h-100 pb-5">
          ...
          <div className="mt-5 mb-2">
            <TalentCardFooter hourlyRate={30} experiencePoints={120} />
          </div>
        </CardBody>
        <hr />
        <CardBody>
          <div>
            <label htmlFor="proposal">Send a message</label>
